...
export default function TagPage() {
  ...
  // ✅ define tagName used by <Seo />
  const tagName = decodeURIComponent(tag || '').replace(/-/g, ' ').trim().replace(/\s+/g, ' ');
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <Seo
        ...
        description={`Guides and posts related to “${tagName}” in Prince George.`}
        image="https://hiddenprincegeorge.ca/og-hero.jpg"
      />

      <h1 className="text-3xl font-bold mb-1 font-serif">
        Stories tagged <span className="text-[#FF7A3F]">#{tagName}</span>
      </h1>
      ...
      {items.length === 0 ? (
        <p>
          ...
          {/* ✅ route to /blog, not /stories */}
          <Link to="/blog" className="text-pg-blue hover:underline">View all stories</Link>.
        </p>
      ) : (
        <div className="space-y-6">
          {items.map((post) => {
            ...
            return (
              ...
              >
                {/* ✅ route to /blog/:slug, not /story/:slug */}
                <Link to={`/blog/${slug}`} className="block">
                  <img src={(post as any).img} alt={post.title} className="w-full h-56 object-cover" loading="lazy" />
                  <div className="p-5">
                    <h2 className="text-2xl font-semibold font-serif">{post.title}</h2>
                    <p className="text-xs text-slate-500 mt-1">{new Date((post as any).date).toLocaleDateString()}</p>
                    <p className="mt-3 text-slate-700">{(post as any).excerpt || 'Full story coming soon.'}</p>
